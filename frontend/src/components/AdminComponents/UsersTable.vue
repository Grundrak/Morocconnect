<template>
  <div :class="{ 'bg-gray-100 text-gray-900': !isDarkMode, 'bg-gray-900 text-white': isDarkMode }"
    ...
    <div class="flex flex-col">
        ...
        <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <tbody
                :class="{ 'bg-white divide-y divide-gray-200': !isDarkMode, 'bg-gray-800 divide-y divide-gray-700': isDarkMode }">
                <tr v-for="user in paginatedUsers" :key="user.id">
                  <td class="px-4 py-2 whitespace-nowrap">
                    <div class="flex items-center">
                      <div class="flex-shrink-0 h-8 w-8">
                        <img v-if="user.avatar" :src="getAvatarUrl(user.avatar)" alt="" class="h-8 w-8 rounded-full">
                        <div v-else
                          class="h-8 w-8 rounded-full bg-gray-300 flex items-center justify-center text-xs text-gray-700">
                          {{ user.initials }}</div>
                      </div>
                      <div class="ml-4">
      ...
      <div class="mt-4 flex justify-between items-center">
        <button @click="prevPage" :disabled="currentPage === 1"
          class="px-3 py-1 bg-gray-200 text-gray-700 rounded-md text-sm disabled:opacity-50">
          Previous
        </button>
