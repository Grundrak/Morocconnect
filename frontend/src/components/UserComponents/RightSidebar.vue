...
<div v-if="currentUser" class="bg-white dark:bg-gray-800 h-full w-64 p-3 flex flex-col">
    ...
    <div class="flex justify-between items-center mb-4 relative">
      <div @click="toggleProfileDropdown" class="flex items-center cursor-pointer ">
        <img :src="avatarSrc" alt="User Avatar" class="w-8 h-8 rounded-full mr-2">
        <div class="text-sm">
      ...
      <div v-if="showProfileDropdown"
        ...
        <div class="py-1">
          ...
          <a @click="goToEditProfile" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700
              cursor-pointer">Edit
            Profile</a>
          <a v-if="isAdmin" @click="goToAdminDashboard" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700
              cursor-pointer">Admin
            Dashboard</a>
          <a @click="logout" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700
              cursor-pointer">Log
            Out</a>
    ...
    <div v-if="!showNotifications">
      <div class="flex justify-between items-center mb-2">
        ...
        <button @click="fetchSuggestedUsers" class="text-xs text-blue-500 font-medium hover:underline bg-transparent">
  ...
  setup() {
    ...
    const goToProfile = () => {
      if (currentUser.value && currentUser.value.id) {
        router.push({ name: 'UserProfil', params: { id: currentUser.value.id } })
      } else {
